...
class OpCacheDataModel
{
    ...
    <style>
        ...
        .op-cache-stats table {
            border-collapse: collapse;
        }

        .op-cache-stats tbody tr:nth-child(even) {
            background-color: #eee;
        }

        .op-cache-stats p.capitalize {
        ...
        .op-cache-stats .tab [type=radio] {
            display: none;
        }

        .op-cache-stats .tab th, .tab td {
            padding: 8px 12px;
        }

        .op-cache-stats .content {
            position: absolute;
            top: 28px;
            left: 0;
            background: white;
        ...
        .op-cache-stats .content table {
            width: 100%;
        }

        .op-cache-stats .content th, .tab:nth-child(3) td {
            text-align: left;
        }

        .op-cache-stats .content td {
            text-align: right;
        }

        .op-cache-stats .clickable {
            cursor: pointer;
        }

        .op-cache-stats [type=radio]:checked ~ label {
            background: white;
            border-bottom: 1px solid white;
            z-index: 2;
        }

        .op-cache-stats [type=radio]:checked ~ label ~ .content {
            z-index: 1;
        }

        .op-cache-stats #graph {
            float: right;
            width: 40%;
            position: relative;
        }

        .op-cache-stats #graph > form {
            position: absolute;
            right: 60px;
            top: -20px;
        }

        .op-cache-stats #graph > svg {
            position: absolute;
            top: 0;
            right: 0;
        }
        ...
        .op-cache-stats #stats th, #stats td {
            padding: 6px 10px;
            font-size: 0.8em;
        }
...
<script>
    ...
    var htmlPartDiv = '<div id="close-partition">&#10006; Close Visualisation</div><div id="partition"></div>';
    if (!$("#partition").length) {
        $('body').append(htmlPartDiv);
    }
