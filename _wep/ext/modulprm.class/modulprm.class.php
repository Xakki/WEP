		...
		try { // ловец снов
			...
			if ($fpath) {
				...
				if (_new_class($Mid, $MODUL, $OWN)) {
					...
					if ($MODUL->verCore) {
						if (version_compare($MODUL->verCore, $this->_CFG['info']['version']) === -1)
							$MESS[] = array('name' => 'error', 'value' => 'Модуль `' . $Mid . '` устарел. Текущая версия ядра `' . $this->_CFG['info']['version'] . '`! ');
					}
...
class modulgrp_class extends kernel_extends
{
	...
	function _create()
	{
		...
		$this->_enum['access'] = array(
			...
			'S' => 'Кнопки сортировки',
			'F' => 'Форма фильтра',
			'R' => 'Переустановка модуля',
			'I' => 'Переиндексация модуля',
			'C' => 'Настроика модуля',
			'H' => 'Проверка структуры модуля',
			'G' => 'Глобальные настройки сервера',
		);
