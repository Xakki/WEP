...
class sqlpostgre
{
    ...
    function sql_connect()
    {
        ...
        $this->hlink = pg_connect('host=' . $this->SQL_CFG['host'] . ' port=5432 dbname=' . $this->SQL_CFG['database'] . ' user=' . $this->SQL_CFG['login'] . ' password=' . $this->SQL_CFG['password']);
        if (!$this->hlink) {
            $_CFG["site"]["work_text"] = '<err>PostgreSQL - ' . static_main::m('Can`t connect to SQL server') . '</err>';
            static_main::downSite();
        }
    ...
    private function _connectDB()
    {
        ...
        if (isset($this->SQL_CFG['setnames']) and $this->SQL_CFG['setnames'])
            pg_query($this->hlink, 'SET NAMES ' . $this->SQL_CFG['setnames']);
    ...
    {
        return pg_query($this->hlink, 'create user \'' . $CFG['login'] . '\'@\'' . $CFG['host'] . '\' identified by \'' . $CFG['password'] . '\'');
    }
    ...
    function longLog($ttt, $sql)
    {
        ...
        if (canShowAllInfo() > 1) {
            ...
            elseif ($ttt > 0.1) $ttt = '<span style="color:#FF6633;">' . $ttt . '</span>'; elseif ($ttt > 0.05) $ttt = '<span style="color:#006699;">' . $ttt . '</span>'; elseif ($ttt > 0.01) $ttt = '<span style="color:#66CCCC;">' . $ttt . '</span>'; elseif ($ttt > 0.005) $ttt = '<span style="color:#006600">' . $ttt . '</span>'; else $ttt = '<span style="color:#00FF00;">' . $ttt . '</span>';
...
class pquery
{
    ...
    { // Выдает асоциативный и нумеровнаый масив
        ...
        elseif ($type == 1)
            return $this->fetch_row($this->handle); elseif ($type == 2)
            return $this->fetch_array($this->handle); else
