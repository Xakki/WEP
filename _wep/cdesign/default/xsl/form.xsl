	...
	<xsl:template name="formelem">
		...
		<tbody>
			...
			<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:for-each select="value">
								 <div class="dateselect">
									...
									<select name="{../@name}[]"><xsl:apply-templates select="item"/></select></div>
								</xsl:for-each>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:if test="comment!=''">
					...
					<td class="td2">
						...
						<xsl:choose>
							...
							<xsl:when test="value!='' and @att_type='img'">
								<div class="wep_thumb">
									<a rel="fancy" href="/{value}" target="_blank" class="fancyimg" title='{@comment}'>
										<img src="/{value}" alt='img' class="attach">
											<xsl:if test="@width"><xsl:attribute name="width"><xsl:value-of select="@width"/></xsl:attribute></xsl:if>
											<xsl:if test="@height"><xsl:attribute name="height"><xsl:value-of select="@height"/></xsl:attribute></xsl:if>
										</img>
									</a>
									<div class="wep_thumb_comment"><xsl:value-of disable-output-escaping="yes" select="@comment"/></div>
								</div>
								<xsl:for-each select="thumb">
									<div class="wep_thumb">
										<a rel="fancy" href="/{.}" target="_blank" class="fancyimg" title='{@comment}'>
											<img src="/{.}" alt='img' class="attach">
												<xsl:if test="../@width"><xsl:attribute name="width"><xsl:value-of select="../@width"/></xsl:attribute></xsl:if>
												<xsl:if test="../@height"><xsl:attribute name="height"><xsl:value-of select="../@height"/></xsl:attribute></xsl:if>
											</img>
										</a>
				...
				<xsl:when test="@type='password_new'">
				  <tr id="tr_{@name}" style="{@style}">
					<td class="td1">
					 	<xsl:value-of disable-output-escaping="yes" select="caption"/>
						<xsl:if test="@min>0"><span style="color:#F00">*</span></xsl:if>
					...
					<td class="td2">
						<xsl:choose>
							<xsl:when test="../@id!=''">
								<input type="password" name="{@name}" value="" class="password"/>
							</xsl:when>
							<xsl:otherwise>
								<input type="password" name="{@name}" value="{value}" class="password"/>
							</xsl:otherwise>
						</xsl:choose>
						
						<div class="passnewdesc" onclick="password_new()">Отобразить символы/скрыть</div>
					</td>
				  </tr>				
				</xsl:when>
						...
						<xsl:choose>
							 ...
							 <xsl:otherwise>
								...
								<div style='width:40%;float:right;'>
									<img src='/_wep/cdesign/default/img/aprm.gif' style='width:18px;cursor:pointer;' onclick='if(confirm("Вы действительно хотите изменить пароль?")) GetId("{@name}").value = hex_md5("{@md5}"+GetId("a_{@name}").value);' alt='Сгенерировать пароль в формате MD5'/>
