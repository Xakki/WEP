	...
	<xsl:template match="topmenu">
		...
		<xsl:for-each select="item">
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<a href="{@href}" onclick="return wep.load_href(this)" class="{@class}">
		...
		<xsl:for-each select="item">
			...
			<td class="ic" style="vertical-align:top;white-space:nowrap;">
				...
				<xsl:if test="count(istree)>0">
					<br/><a href="?{../req}{../cl}_id={id}" onclick="return wep.load_href(this)"><xsl:value-of select="istree"/> (<xsl:value-of select="istree/@cnt"/>)</a>
				</xsl:if>
				<xsl:for-each select="child">
					<br/><a href="?{../../req}{../../cl}_id={../id}&amp;{../../cl}_ch={@name}" onclick="return wep.load_href(this)"><xsl:value-of select="."/> (<xsl:value-of select="@cnt"/>)</a>
				</xsl:for-each>
			</td>
			  <!--<td class="ic">
		...
		<xsl:if test="count(link)>0">
			<div class="pagenum">
			  <xsl:for-each select="link">
				...
				<xsl:otherwise>
					<a href="{@href}" onclick="return wep.load_href(this)"><xsl:value-of select="."/></a>
				</xsl:otherwise>
				</xsl:choose>
			 </xsl:for-each>
			 &#160;
