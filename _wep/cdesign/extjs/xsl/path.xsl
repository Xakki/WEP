...
<xsl:output method="xml" indent="no" omit-xml-declaration = "yes" standalone = "yes" doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN" doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-strict.dtd"/>
	...
	<xsl:if test="count(item)>0">
		...
		<xsl:for-each select="item">
			...
			<xsl:if test="position()=last()"><xsl:value-of disable-output-escaping="yes" select="name"/>&#160;<a class="bottonimg imgf5" href="{href}" onclick="return wep.load_href(this)"></a></xsl:if>
		</xsl:for-each>
		</div>
	</xsl:if>
	</xsl:template>
