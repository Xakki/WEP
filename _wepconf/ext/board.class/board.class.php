...
class board_class extends kernel_class {
	...
	protected function _create_conf() {/*CONFIG*/
		...
		$this->config_form['onComm'] = array('type' => 'checkbox', 'caption' => 'Включить комментарии?');
	...
	protected function _create() {
		...
		$this->fields_form['city'] = array(
			'type' => 'ajaxlist',
			'lable'=>'Введите название города или региона',
		...
		if($this->config['onComm'])
			$this->fields_form['on_comm'] = array('type' => 'checkbox', 'caption' => 'Включить комментарии?', 'css'=>'hideparams', 'mask'=>array('fview'=>1));
		...
		while(isset($this->config['nomination'.$i])) {
			if($this->config['nomination'.$i]!='') {
				...
				$this->fields_form['nomination'.$i] = array('type' => 'int', 'caption' => 'Номинация : '.$this->config['nomination'.$i],'readonly'=>1, 'mask' =>array('filter'=>1,'sort'=>1,'usercheck'=>2));//'fview'=>1,
			}
			$i++;
		}
		...
		if(_prmUserCheck()) {
			...
			$this->_enum['datea']=array(
				...
				86400=>'через 1 день',
				172800=>'через 2 дня');
			$this->fields_form['period']['comment']='Зарегестрированным срок размещения до - 90 дней';
			$this->fields_form['datea']['comment']='Зарегестрированным дата публикации до - 14 дней';
		}
		...
		if($this->_autoCheckMod or $_GET['_type']=='checkmodul') {
			include_once($this->_CFG['_PATH']['ext'].'board.class/childs.class.php');
	...
	public function kPreFields(&$data,&$param) {
		...
		$this->fields_form['city'] = array(
			...
			'value'=>$valcity, 
			'mask' =>array('min'=>1), 
			'caption' => 'Ваш город <a class="selectcity" href="/city.html" onclick="return JSHRWin(\''.$this->_CFG['_HREF']['siteJS'].'?_view=addcity\',\'\')">'.(!$namecity?'Выбирите город':$namecity).'</a>');*/
	...
	public function kFields2Form(&$data,&$param) {
		$flag = parent::kFields2Form($data,$param);
		if($this->id) {
...
WHERE t1.owner_id IN ('.implode(',',$FLI).') and t1.active=1 GROUP BY t1.id ORDER BY t1.ordind';
				...
				if(!$result->err)
					while ($row = $result->fetch_array()) {
						if($flagNew and !isset($this->_enum['fli'.$row['owner_id']][0][0]))
						...
						unset($row['elem']);
						$this->_enum['fli'.$row['owner_id']][0][$row['id']] = $row;
	...
	public function fListDisplay($rid,$filter,$rss=0,$order='t1.datea',$limit=0) {
		...
		if(!$result->err and $row = $result->fetch_array() and $row['cnt']>0) {
			...
			if(!$rss) {
				if(!isset($_COOKIE['checkloadfoto'])){
					$_COOKIE['checkloadfoto']=1;
					setcookie('checkloadfoto',1, (time()+$this->_CFG['remember_expire']));
				}
				$xml .= '<imcookie>'.$_COOKIE['checkloadfoto'].'</imcookie>';
			}
		}
		return $xml;
	}
...
//$newtime = getmicrotime();
		...
		if($flag) {
			...
			$this->filter_form['rubric']= array(
				...
				'listname'=>'rubrics',
				'caption'=>'Рубрика',
				'onchange'=>'JSHRWin(\''.$this->_CFG['_HREF']['siteJS'].'?_view2=subscribeparam\',{\'rubric\':this.value})');
			if(isset($filter['rubric']))
				$this->filter_form['rubric']['value']=$filter['rubric'];
		}
