...
*/
    ...
    static function _resizeImage($InFile, $OutFile, $WidthX, $HeightY)
    {
        ...
        if ($imtype > 3) {
            static_main::log(
                'alert',
                'Данный тип изображения не поддерживается на данный момент, рекомендуем использовать JPEG, PNG или GIF'
            );
            copy($InFile, $OutFile);
            return true;
        }
    ...
    static function _cropImage($InFile, $OutFile, $WidthX, $HeightY)
    {
        ...
        if (!$WidthX) $WidthX = ($width_orig * $HeightY) / $height_orig;
        if (!$HeightY) {
            $HeightY = ($height_orig * $WidthX) / $width_orig;
        }
        ...
        imagecopyresampled(
            ...
            0,
            0,
            $width_orig / 2 - $WidthX / 2,
            $height_orig / 2 - $HeightY / 2,
            $WidthX,
            $HeightY,
            $WidthX,
            $HeightY
...
//        $src_y = $HeightY/2 - $trueY/2;
        ...
        if (!($thumb = imagecreatetruecolor($WidthX, $HeightY))) return static_main::log('error', 'Cannot Initialize new GD image stream');
...
16	IMAGETYPE_XBM
		*/
        if ($imtype == 1) {
            if (!($image = @imagecreatefromgif($im_file))) return static_main::log('error', 'Can not create a new image from file');
        } elseif ($imtype == 2) {
            if (!($image = imagecreatefromjpeg($im_file))) return static_main::log('error', 'Can not create a new image from file');
        } elseif ($imtype == 3) {
            if (!($image = imagecreatefrompng($im_file))) return static_main::log('error', 'Can not create a new image from file');
        } else return false;
        return $image;
    }
