...
class static_image
{
    ...
    static function _waterMark($InFile, $OutFile, $logoFile = '', $posX = 0, $posY = 0)
    {
        ...
        if (self::hasMagick()) {
            if (strpos($posX, '%') !== false) $posX = $width_orig * substr($posX, 0, -1) / 100;
            if (strpos($posY, '%') !== false) $posY = $height_orig * substr($posY, 0, -1) / 100;
    ...
    static function _cropImage($InFile, $OutFile, $WidthX, $HeightY, $posX = 0, $posY = 0)
    {
        ...
        if (self::hasMagick()) {
            ...
            if ($err) {
                ...
                $res = static_imageGD2::_cropImage($InFile, $OutFile, $WidthX, $HeightY, $posX, $posY);
            }
        }
    ...
    static function convertImage($InFile, $OutFile, $WidthX, $HeightY)
    {
        ...
        if (self::hasMagick()) { ///// todo not work yet
            ...
            $crop = true;
            $thumb = new Imagick($InFile);
            if ($crop) $thumb->cropThumbnailImage($WidthX, $HeightY);
            else $thumb->thumbnailImage($WidthX, $HeightY, true);
            $res = $thumb->writeImage($OutFile);
            $thumb->destroy();
        } elseif (self::hasConsoleMagick()) {
            _chmod($InFile);
            $crop = true;
            if ($crop) $cmd = 'convert ' . escapeshellarg($InFile) . ' -resize "' . $WidthX . 'x' . $HeightY . '^" -gravity center -crop ' . $WidthX . 'x' . $HeightY . '+0+0 +repage  ' . escapeshellarg($OutFile);
            else $cmd = 'convert ' . escapeshellarg($InFile) . ' -thumbnail "' . $WidthX . 'x' . $HeightY . '" ' . escapeshellarg($OutFile);
            $out = array();
            $err = 0;
            $run = exec($cmd, $out, $err);
    ...
    static function getActiualSize($InFile, $Width, $Height, $saveOrigin = false)
    {
        ...
        if (!$Width) $Width = (int)($width_orig * $Height / $height_orig);
        if (!$Height) $Height = (int)($height_orig * $Width / $width_orig);
    ...
    static function hex2rgb($str_color)
    {
        ...
        if (strlen($str_color) == 6) list($r, $g, $b) = array(
            $str_color[0] . $str_color[1],
            $str_color[2] . $str_color[3],
            $str_color[4] . $str_color[5]
        );
        elseif (strlen($str_color) == 3) list($r, $g, $b) = array($str_color[0] . $str_color[0], $str_color[1] . $str_color[1], $str_color[2] . $str_color[2]);
        else return false;
