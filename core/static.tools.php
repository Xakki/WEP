...
class static_tools
{
	    ...
	    static function _installTable(&$MODUL)
    {
        ...
        if (!$flag) {
            ...
            if (!$MODUL->SQL->_tableCreate($MODUL)) {
                static_main::log(
                    'error',
                    'Для модуля `' . $MODUL->caption . '` не удалось создать таблицу.',
                    $MODUL->_cl
                );
                return false;
            }
            if (!self::_insertDefault($MODUL)) {
                ...
                static_main::log(
                    'error',
                    'Для модуля `' . $MODUL->caption . '` не удалось записать дефолтные данные, и поэтому таблица не будет создана.',
                    $MODUL->_cl
                );
                return false;
            }
	    ...
	    static function _checkTable(&$MODUL)
    {
        $rDATA = array();
        if (!$MODUL->tablename) {
            $rDATA['Создание таблицы']['@mess'][] = array(
                'notice', 'Модуль `' . $MODUL->caption . '`[' . $MODUL->_cl . '] не использует базу данных.'
            );
            return $rDATA;
        }
            ...
            if (isset($_POST['sbmt'])) {
                if (!$MODUL->SQL->_tableCreate($MODUL)) {
                    ...
                    if (!self::_insertDefault($MODUL)) {
                        $MODUL->SQL->_tableDelete($MODUL->tablename);
                        $rDATA['Создание таблицы']['@mess'][] = array(
                            'error', 'Для модуля `' . $MODUL->caption . '` не удалось записать дефолтные данные, и поэтому таблица не будет создана.'
                        );
                    } else $rDATA['Создание таблицы']['@mess'][] = array('notice', 'Для модуля `' . $MODUL->caption . '` успешно создана таблица.');
                }
            } else {
        ...
        foreach ($dataTable as $fldname => $fp) {
            ...
            } elseif (isset($MODUL->attaches[$fldname])) $MODUL->attaches[$fldname]['inst'] = '1';
            elseif (isset($MODUL->memos[$fldname])) $MODUL->memos[$fldname]['inst'] = '1';
            else $rDATA[$fldname]['@newquery'] = 'ALTER TABLE `' . $MODUL->tablename . '` DROP `' . $fldname . '`';
        }
        ...
        if (isset($MODUL->unique_fields) and count($MODUL->unique_fields)) {
            foreach ($MODUL->unique_fields as $k => $r) {
                ...
                if (!isset($uniqlist[$k])) { // and !isset($uniqlistR[$k])
                    foreach ($r as $kk => $rr) $uniqlistR[$k][$kk] = $rr;
                    $tmp = '';
        ...
        if ($MODUL->mf_istree) {
            ...
            if ($isset) $isCorrect = self::_nestedSets_isCorrect($MODUL);
    ...
    static function _save_config($conf, $file)
    {
        foreach ($conf as &$r) {
            if (is_string($r) and strpos($r, ':|') !== false) {
                ...
                foreach ($temp as $t => $d) {
                    $temp2 = explode(':=', $d);
                    if (count($temp2) > 1) $r[trim($temp2[0])] = trim($temp2[1]);
                    else $r[] = trim($d);
                }
            }
        }
	    ...
	    static public function toolsReinstall($_this)
    {
        ...
        if (!static_main::_prmModul($_this->_cl, array(11))) $RESULT['messages'][] = static_main::am('error', 'denied', $_this);
	    ...
	    static public function toolsConfigmodul($_this)
    {
        ...
        if (!static_main::_prmModul($_this->_cl, array(13))) {
            ...
            if (count($_POST)) {
                ...
                } else $RESULT['messages'] = array_merge($RESULT['messages'], $arr['mess']);
            } else {
                ...
                foreach ($_this->config_form as $k => $r) {
                    if (isset($_this->config[$k])) {
                        if (!is_array($_this->config[$k])) $_this->config_form[$k]['value'] = stripslashes($_this->config[$k]);
                        else $_this->config_form[$k]['value'] = $_this->config[$k];
                    }
                }
	    ...
	    static public function toolsSuperGroup(&$_this)
    {
        global $_tpl;
        $fields_form = $mess = array();
        if (!static_main::_prmModul($_this->_cl, array(5, 7))) $mess[] = static_main::am('error', 'denied', $_this);
        elseif (!isset($_COOKIE['SuperGroup'][$_this->_cl]) or !count($_COOKIE['SuperGroup'][$_this->_cl])) $mess[] = static_main::am('alert', 'Нет выбранных элементов', $_this);
        elseif (count($_POST)) {
            $type = '';
            if (isset($_POST['sbmt_on'])) {
                ...
                $prevId = $_this->id;
                $_this->id = array_keys($_COOKIE['SuperGroup'][$_this->_cl]);
                if (self::copyModuleDataByID($_this)) $mess[] = static_main::am('ok', 'Скопировано', $_this);
                $_this->id = $prevId;
            }

            if (count($mess)) {
                foreach ($_COOKIE['SuperGroup'][$_this->_cl] as $ck => $ck) $_tpl['onload'] .= 'setCookie("SuperGroup[' . $_this->_cl . '][' . $ck . ']",0,-10000);';
    ...
    static private function saveCopyModuleData($MODUL, $data)
    {
        ...
        if (!count($data)) return false;
        if (!$MODUL->_add($data, false)) return false;
        $newId = $MODUL->id;
	    ...
	    static function toolsStatsmodul(&$MODUL, $oid = '')
    {
        ...
        $clause = array();
        if (!$oid and isset($_GET['_oid'])) $oid = (int)$_GET['_oid'];
        if ($oid) $clause[] = 't1.' . $MODUL->owner_name . '=' . $oid;
        ...
        foreach ($multiline as $kLine => $rLine) {
            ...
            if (count($clauseTemp)) $clauseTemp = 'WHERE ' . implode(' and ', $clauseTemp);
            else $clauseTemp = '';
	    ...
	    static function getFdata($file, $start = '', $end = '', $mData = false)
    {
        ...
        if (!file_exists($file)) return $_CFG;
        $fc = '';
        if ($start == '' and $end == '') {
        ...
        if ($fc) eval($fc);
        else trigger_error('NO CFG', E_USER_WARNING);
    ...
    static function saveUserCFG($SetDataCFG)
    {
        ...
        if (isset($SetDataCFG['wep'])) {
            if (!isset($SetDataCFG['wep']['password']) or !$SetDataCFG['wep']['password'] or $SetDataCFG['wep']['password'] == $DEF_CFG['wep']['password']) {
                $mess[] = array(
                    'error', 'Поле ' . $_CFGFORM['wep']['password']['caption'] . ' обязательное и не должно совпадать с дефолтным'
                );
            }
            if (!isset($SetDataCFG['wep']['md5']) or !$SetDataCFG['wep']['md5'] or $SetDataCFG['wep']['md5'] == $DEF_CFG['wep']['md5']) {
                $mess[] = array(
                    'error', 'Поле ' . $_CFGFORM['wep']['md5']['caption'] . ' обязательное и не должно совпадать с дефолтным'
                );
            }
        }
        ...
        if (count($mess)) return array($fl, $mess);
    ...
    static function saveCFG($SetDataCFG, $file, $DEF_CFG = array())
    {
        ...
        foreach ($DEF_CFG as $k => $r) {
            foreach ($r as $kk => $defr) {
                $newr = '';
                $flag = false;
                if (isset($SetDataCFG[$k][$kk])) $newr = $SetDataCFG[$k][$kk];
                elseif (isset($USER_CFG[$k][$kk])) $newr = $USER_CFG[$k][$kk];
        ...
        if (!file_put_contents($file, $putFile)) {
            ...
            $fl = true;
            if (isset($SetDataCFG['sql'])) $mess[] = array('ok', 'Подключение к БД успешно.');
	    ...
	    static function _insertDefault(&$MODUL)
    {
        ...
        if (file_exists($file)) {
            ...
            foreach ($lines as $line) {
                $line = trim($line, "\s\;");
                if ($line) {
                    if (!$MODUL->SQL->execSQL($line)) return false;
                }
            }
        }
	    ...
	    static function _checkdir($dir)
    {
        ...
        if (!file_exists($dir)) {
            ...
            if (!mkdir($dir, $_CFG['wep']['chmod'], true)) return static_main::log('error', 'Cannot create directory <b>' . $dir . '</b>');
        } else {
            _chmod($dir);
            $f = fopen($dir . '/test.file', 'w');
            if (!$f) return static_main::log('error', 'Cannot create file `test.file` in directory `' . $dir . '`');

            $err = fwrite($f, 'zzz') == -1;
            fclose($f);
            unlink($dir . '/test.file');

            if ($err) return static_main::log('error', 'Cannot write/read file `test.file` in directory `' . $dir . '`');
        }
        return true;
    }
...
deny from all
        ...
        if (!file_exists($_CFG['_PATH']['controllers'] . 'cron.php')) {
            file_put_contents(
                $_CFG['_PATH']['controllers'] . 'cron.php',
                '<?php
	    ...
	    static function _toolsCheckmodul(&$MODUL)
    {
        ...
        if (!static_main::_prmModul($MODUL->_cl, array(14))) $mess[] = array('error', 'Access denied');
        else {
...
//							$select['nestedSets'] = true;
                ...
                foreach ($row as $kk => $rr) {
                    ...
                    if (!is_array($rr)) $desc = $rr;
                    elseif (isset($rr['@newquery']) and isset($rr['@oldquery'])) $desc = 'Было: ' . _e($rr['@oldquery']) . '<br/>Будет: ' . _e($rr['@newquery']);
                    elseif (isset($rr['@newquery'])) $desc = _e($rr['@newquery']);
                    else $desc = '';
                    if ($desc) $valuelist[$kk . '@newquery'] = '<i>' . $kk . '</i> - ' . $desc;

                    if (is_array($rr) and isset($rr['@index'])) $valuelist[$kk . '@index'] = '<i>' . $kk . '</i> - ' . $rr['@index'];
                }
                ...
                if (count($valuelist)) {
                    ...
                    if ($value) $form['query_' . $_cl]['value'] = $value;
	    ...
	    static private function _toolsCheckmodulFormPost(&$MODUL, &$check_result, &$mess)
    {
        ...
        foreach ($check_result as $_cl => $row) {
            if (isset($row['@reattach']) and isset($_POST['query_' . $_cl]['reattach'])) {
                ...
                if (self::_reattaches($MODUL_R)) $mess[] = array('ok', '<b>' . $_cl . '</b> - ' . static_main::m('_file_ok', $MODUL));
                else {
            ...
            if (isset($row['@nestedSets']) and isset($_POST['query_' . $_cl]['nestedSets'])) {
                _new_class($_cl, $MODUL_R);
                if (self::_nestedSets_fullUpdate($MODUL_R, $mess)) $mess[] = array('ok', '<b>' . $_cl . '</b> - ' . static_main::m('_nestedSets_fullUpdate ok', $MODUL));
                else {
	    ...
	    static function _nestedSets_fullUpdate(&$MODUL, &$mess)
    {
        ...
        if ($MODUL->mf_ordctrl) {
            $result = $MODUL->qs(
                'id,' . $MODUL->mf_istree . ',' . $MODUL->mf_ordctrl . ',' . $MODUL->ns_config['left'] . $rootKey,
                'ORDER BY ' . $MODUL->mf_ordctrl,
                'id',
                $MODUL->mf_istree
            );
        } else {
            $result = $MODUL->qs(
                'id,' . $MODUL->mf_istree . ',' . $MODUL->ns_config['left'] . $rootKey,
                ' ORDER BY ' . $MODUL->ns_config['left'] . '',
                'id',
                $MODUL->mf_istree
            );
        }
...
//        print_r('</pre>');
                ...
                if (zip_entry_open($zip, $zip_entry, "r")) {
                    if (preg_match('#^' . $dirFromZip . '.*#', dirname(zip_entry_name($zip_entry)))) {
                        if (substr($completeName, -1) != '/' and $fd = @fopen($completeName, 'w+')) {
                            ...
                            if (!file_exists(rtrim($completeName, '/'))) mkdir($completeName, 0777);
                        }
    ...
    static function transliteRuToLat($var, $len = 0)
    {
        ...
        $var = strtr($var, array('-----' => '-', '----' => '-', '---' => '-', '--' => '-'));
        if ($len) $var = mb_substr($var, 0, $len, 'UTF-8');
        return trim($var, ",. -=");
    }
...
//			}
    ...
    {
        $fi = array(
            'name' => '', 'desc' => '', 'ShowFlexForm' => false, 'type' => '', 'tags' => '', 'ico' => 'defaul.png', 'author' => '', 'version' => '',
            'return' => ''
        );
            ...
            foreach ($fcontent as $r) {
                ...
                if ($r) {
                    ...
                    if ($temp[0] and $temp[0][0] == '@') {
                        ...
                        array_shift($temp);
                        $fi[$nm] = implode(' ', $temp);
                        if ($fi[$nm] === 'true') $fi[$nm] = true;
                        elseif ($fi[$nm] === 'false') $fi[$nm] = false;
                    } elseif (!$fi['name']) $fi['name'] = $r;
                    else $fi['desc'] .= $r;
                }
            }
        }
        ...
        foreach ($info as $k => $r) {
            if (isset($data[$k]) and _new_class($r['class'], $MODEL)) {
                foreach ($data[$k] as $row) {
                    ...
                    if (isset($r['setowner'])) $insertData[$r['setowner']] = $owner;
                    ...
                    if ($insertData['id']) self::helperImport1C($info, $row, $insertData['id']);
                }
            }
        }
    }
    ...
    static function runc($cmd)
    {
        ...
        if (!count($output)) $html .= '<div style="color:blue;">' . $last_line . '</div>';
        else foreach ($output as $row) $html .= '<div style="color:gray;">' . $row . '</div>';
        if ($retval) $html .= '<h4>exec status = ' . $retval . '</h4>';
