...
function fileInclude()
{
    ...
    if (isset($_tpl['script'][getUrlScript('wepform')])) $_tpl['script'] = array(getUrlScript('wepform') => 1) + $_tpl['script'];
...
function arraySrcToStr()
{
    ...
    if ($_CFG['site']['production']) $_tpl['onload'] = ' window.isProduction = true; ' . $_tpl['onload'];
...
function scriptRecursive($script, $solt = '')
{
    ...
    foreach ($script as $kk => $rr) {
        if (is_string($rr) and substr($rr, 0, 1) == '<') {
            trigger_error(
                'Обнаружена не совместимость: ошибка загрузки скриптов `' . $kk . '` - `' . $rr . '`',
                E_USER_WARNING
            );
        }

        if (is_string($kk) and isUrl($kk)) {
            $temp .= '<script src="' . soltAppend($solt, $kk) . '' . ($rr == 'async' ? 'async' : '') . '"></script>' . "\n";
        }
...
function cssRecursive($css, $solt = '')
{
    ...
    foreach ($css as $kk => $rr) {
        if (is_string($rr) and substr($rr, 0, 1) === '<') {
            trigger_error(
                'Обнаружена не совместимость: ошибка загрузки стилей `' . $kk . '` - `' . $rr . '`',
                E_USER_WARNING
            );
        }
        ...
        if (is_string($rr) and $rr) {
            if (isUrl($rr)) $temp .= '<link rel="stylesheet" href="' . soltAppend($solt, $rr) . '"/>' . "\n";
            else $temp .= "<style>" . $rr . "</style>\n";
        } elseif (is_array($rr)) $temp .= cssRecursive($rr, $solt);
    }
    return $temp;
}
