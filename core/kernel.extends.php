...
abstract class kernel_extends
{
        ...
        foreach ($modul as $m) {
            if (isset($this->_CFG['hook'][$f][$m])) {
                foreach ($this->_CFG['hook'][$f][$m] as $k => $r) {
                    ...
                    } else trigger_error('Для модуля `' . $this->_cl . '`, функция хука `' . $r . '` не определена', E_USER_WARNING);
                }
            }
        }
        return false;
    ...
    protected function configParse()
    {
        if (isset($this->config_form)) { // загрузка конфига из файла для модуля
            if (is_null($this->_file_cfg)) $this->_file_cfg = $this->_CFG['_PATH']['configDir'] . get_class($this) . '.cfg';
            if (file_exists($this->_file_cfg)) {
                $cont = file_get_contents($this->_file_cfg);
                if (substr($cont, 0, 5) == 'array') eval('$data=' . $cont . ';');
                else $data = static_main::_fParseIni($this->_file_cfg, $this->config_form);
    ...
    {
        if ($this->tablename) $this->tablename = $this->SQL_CFG['dbpref'] . $this->tablename; // название таблицы
        if (is_bool($this->mf_namefields) and $this->mf_namefields) $this->mf_namefields = 'name';
        if (is_bool($this->mf_createrid) and $this->mf_createrid) $this->mf_createrid = 'creater_id';
        if (is_bool($this->mf_istree) and $this->mf_istree) $this->mf_istree = 'parent_id';
        if (is_bool($this->mf_ordctrl) and $this->mf_ordctrl) $this->mf_ordctrl = 'ordind';
        if (is_bool($this->mf_actctrl) and $this->mf_actctrl) $this->mf_actctrl = 'active';
        if (is_bool($this->mf_timecr) and $this->mf_timecr) $this->mf_timecr = 'mf_timecr';
        if (is_bool($this->mf_timestamp) and $this->mf_timestamp) $this->mf_timestamp = 'mf_timestamp';
        if (is_bool($this->mf_ipcreate) and $this->mf_ipcreate) $this->mf_ipcreate = 'mf_ipcreate';
        if (is_bool($this->mf_notif) and $this->mf_notif) $this->mf_notif = 'mf_notif';
        ...
        } else $this->fields['id'] = array('type' => 'int', 'width' => 11, 'attr' => 'UNSIGNED NOT NULL AUTO_INCREMENT');

        if ($this->mf_namefields) $this->fields[$this->mf_namefields] = array('type' => 'varchar', 'width' => '255', 'attr' => 'NOT NULL', 'default' => '');

        if ($this->owner) {
            $this->fields[$this->owner_name] = $this->owner->fields['id'];
            if (stripos($this->fields[$this->owner_name]['attr'], 'UNSIGNED') !== false) $this->fields[$this->owner_name]['attr'] = 'UNSIGNED NOT NULL';
            else $this->fields[$this->owner_name]['attr'] = 'NOT NULL';
            if ($this->owner_unique) $this->unique_fields[$this->owner_name] = $this->owner_name;
            else $this->index_fields[$this->owner_name] = $this->owner_name;

            if (stripos($this->fields[$this->owner_name]['type'], 'int') !== false) $this->fields[$this->owner_name]['default'] = 0;
            else $this->fields[$this->owner_name]['default'] = '';
        }
        ...
        if ($this->mf_istree) {
            ...
            if ($this->mf_use_charid) $this->fields[$this->mf_istree]['default'] = '';
            else $this->fields[$this->mf_istree]['default'] = '0';
        ...
        if ($this->mf_timestamp) $this->fields[$this->mf_timestamp] = array('type' => 'timestamp', 'attr' => 'NOT NULL');
        if ($this->mf_timecr) $this->fields[$this->mf_timecr] = array('type' => 'int', 'width' => 11, 'attr' => 'unsigned NOT NULL', 'default' => '0');
        if ($this->mf_timeup) $this->fields['mf_timeup'] = array('type' => 'int', 'width' => 11, 'attr' => 'unsigned NOT NULL', 'default' => '0');
        if ($this->mf_timeoff) $this->fields['mf_timeoff'] = array('type' => 'int', 'width' => 11, 'attr' => 'unsigned NOT NULL', 'default' => '0');
        if ($this->mf_ipcreate) $this->fields[$this->mf_ipcreate] = array('type' => 'bigint', 'noquote' => true, 'width' => 20, 'attr' => 'NOT NULL', 'default' => '0');
...
//						include_once($this->_CFG['_PATH']['ext'].$this->_cl.'.class/'.$r.'.childs.php');
                elseif (file_exists($this->_CFG['_PATH']['wep_ext'] . $this->_cl . '.class/' . $r . '.childs.php')) $this->child_path[$r] = $this->_CFG['_PATH']['wep_ext'] . $this->_cl . '.class/' . $r . '.childs.php';
//					include_once($this->_CFG['_PATH']['wep_ext'].$this->_cl.'.class/'.$r.'.childs.php');
    ...
    public function queryEscape($where, $union = 'and')
    {
        if (is_array($where)) {
            ...
            foreach ($where as $k => $r) {
                if (isset($this->fields[$k])) $where2[$k] = '`' . $k . '`="' . $this->SqlEsc($r) . '"';
            }
            if (count($where2)) $where = 'WHERE ' . implode(' ' . $union . ' ', $where2);
            else $where = '';
        } elseif (isint($where)) {
    ...
    { // this func. dont use $this->data
        ...
        foreach ($this->fields as $kf => $rf) {
            if (isset($rf['secure'])) $secureKey[] = $kf;
        }
        if ($ord != '' and $ord2 != '') {
            while ($row = $result->fetch()) {
                if (isset($secureKey)) {
                    foreach ($secureKey as $sk) $row[$sk] = static_main::EnDecryptString($row[$sk]);
                }
        ...
        if (count($data) and !$this->_select_attaches($data)) trigger_error('Ошибка выборки ATTACHES в классе ' . $this->_cl, E_USER_WARNING);
        if (count($data) and !$this->_select_memos($data)) trigger_error('Ошибка выборки MEMOS в классе ' . $this->_cl, E_USER_WARNING);
        if (isset($this->_CFG['hook']['_query'])) $this->__do_hook('_query', func_get_args());
        return $data;
    }
    ...
    public function count($cls = '', $debug = false)
    {
        ...
        if (count($result)) return $result[0]['cnt'];
        else return 0;
    }
    ...
    private function _select_fields($sql = null, $simple = false)
    {
        ...
        if (!is_null($sql)) {
            if (is_array($sql)) {
                if (isset($sql['select']) and $sql['select']) $q_select = $sql['select'];
                if (isset($sql['where']) and $sql['where']) $q_where[] = $sql['where'];
                if (isset($sql['order']) and $sql['order']) $q_order = $sql['order'];
                if (isset($sql['limit']) and $sql['limit']) $q_limit = $sql['limit'];
            } elseif ($sql) $q_where[] = $sql;
        }

        if (count($q_where)) $q_where = ' WHERE ' . implode(' and ', $q_where);
        else $q_where = ' ';

        if ($q_order) $q_order = ' ORDER BY ' . $q_order;

        if ($q_limit) $q_limit = ' LIMIT ' . $q_limit;
        ...
        if (!$simple) {
            ...
            foreach ($data as &$row) {
                foreach ($listAr as $k => $r) {
                    if (isset($r[$row[$k]])) $row['#' . $k . '#'] = $r[$row[$k]];
                }
            }
        }
        ...
        if (isset($this->id) and $this->id) {
            reset($data);
            if (count($data) == 1) $this->id = key($data);
            elseif (count($data) > 1) $this->id = array_keys($data);
            else $this->id = NULL;
        }
        return $data;
    }
    ...
    { // TODO const
        ...
        if (count($list)) {
            ...
            if (count($subList)) $list = array_merge($list, $subList);
        }
        return $list;
    }
    ...
    public function _add($data = array(), $flag_select = true, $flag_update = false)
    {
        if (is_bool($data)) {
            ...
            foreach ($data as $k => $r) {
                if (isset($this->memos[$k])) $this->mmo_data[$k] = $r;
                elseif (isset($this->attaches[$k])) $this->att_data[$k] = $r;
    ...
    public function _get_new_ord()
    {
        $query = 'SELECT max('
            ...
            . ') FROM `' . $this->tablename . '`';
        if ($this->mf_istree and $this->parent_id and !$this->fld_data[$this->mf_istree]) $query .= ' WHERE ' . $this->mf_istree . '=' . $this->parent_id;
        $result = $this->exec($query);
        if ($result === false) return 0;
        list($ordind) = $result->fetch_row();
        if ($ordind = (int)$ordind) $ordind++;
        return $ordind;
    }
...
// *** PERMISSION ***//
    ...
    public function _prmModulAdd()
    {
        ...
        if (static_main::_prmModul($this->_cl, array(9))) return true;
        return false;
    }
...
// --END PERMISSION -----//
    ...
    public function kPreFields(&$f_data, &$f_param = array(), &$f_fieldsForm = null)
    {
        ...
        foreach ($f_fieldsForm as $k => &$r) {
            ...
            if ($eval) {
                if (isset($f_data[$k])) $val = $f_data[$k]; // Переменная используемая в eval
                else $val = '';
            ...
            if (!isset($r['fields_type'])) {
                if (isset($this->fields[$k])) $r['fields_type'] = $this->fields[$k]['type'];
                else $r['fields_type'] = $r['type'];
            }
            ...
            if ($k == $this->owner_name and !isset($f_data[$k])) {
                ...
                elseif (!isset($this->parent_id) and $this->mf_use_charid) $this->parent_id = '';
                elseif (!isset($this->parent_id)) $this->parent_id = 0;
            } elseif ($r['type'] == 'ckedit') {
                if (isset($this->memos[$k]) and !count($_POST) and file_exists($f_data[$k])) $r['value'] = file_get_contents($f_data[$k]);
                elseif (isset($f_data[$k])) $r['value'] = $f_data[$k];
                ...
                if (isset($f_data[$k])) {
                    ...
                    } else $r['value'] = $f_data[$k];
                    $r['value'] = array_combine($r['value'], $r['value']); // На всякий, иногда эта функция может самостоятельно работать
                }
            } elseif ($r['type'] == 'date') {
                if (!isset($r['mask']['format']) or !$r['mask']['format']) $r['mask']['format'] = 'Y-m-d H:i:s';
            } elseif ($r['type'] == 'password') {
                if ($this->id) {
                    if (!isset($r['mask']['password'])) $r['mask']['password'] = 'change';
                    $r['value'] = '';
                }
        ...
        if (!isset($f_param['captchaOn'])) {
            if (!isset($_SESSION['user']['id'])) $f_param['captchaOn'] = true;
            else $f_param['captchaOn'] = false;
        }
        ...
        $mess = array();
        if (isset($this->mess_form) and count($this->mess_form)) $mess = $this->mess_form;

        if (!count($f_fieldsForm)) $mess[] = static_main::am('error', 'nodata', [], $this);

        if (isset($this->_CFG['hook']['kPreFields'])) $this->__do_hook('kPreFields', func_num_args());
        return $mess;
    }
    ...
    function getFieldsForm($form = 0)
    {
        ...
        if ($this->cf_fields and is_array($this->config['cf_fields']) and count($this->config['cf_fields'])) {
            foreach ($this->config['cf_fields'] as $fk => $fr) {
                if (isset($fr['ftype'])) $fr['type'] = $fr['ftype'];
        ...
        if ($form == 0 and count($this->formDSort)) {
            ...
            if (is_array(current($this->formSort))) {
                foreach ($this->formSort as $tr) {
                    if (is_array($tr)) $formSort = array_merge($formSort, $tr);
                    else $formSort[] = $tr;
                }
            } else $formSort = $this->formSort;
            $temp = $this->fields_form;
    ...
    public function getFormOptions($name = null, $method = null)
    {
        ...
        $option = array(
            'name' => $name, 'method' => $method, 'id' => $this->id, 'action' => $_SERVER['REQUEST_URI'], 'prevhref' => (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '')
        );
        return $option;
    }
    ...
    public function kFields2Form(&$param, &$fields_form = null)
    {
        ...
        if (!isset($fields_form['_info'])) {
            ...
            if ($this->id) $fields_form['_info']['caption'] = static_main::m('update_name', array($this->caption), $this);
            else $fields_form['_info']['caption'] = static_main::m('add_name', array($this->caption), $this);
        }
        ...
        if (!isset($fields_form['sbmt'])
            ...
            if (isset($param['sbmt_save'])) $sbmtList['sbmt_save'] = static_main::m(($this->id ? 'Save' : 'Save and add'), $this);
            if (isset($param['sbmt_close'])) $sbmtList['sbmt_close'] = array('#name#' => static_main::m('Close', $this));
            if ($this->id and $this->_prmModulDel($this->data, $param) and isset($param['sbmt_del'])) $sbmtList['sbmt_del'] = array('#name#' => static_main::m('Delete', $this), 'confirm' => 'Подтвердите удаление!');
...
// MODUL configuration
    ...
    public function confirmForm($param = array())
    {
        ...
        if (!isset($param['confirmValue'])) $param['confirmValue'] = '1';
        if (!isset($param['savePost'])) $param['savePost'] = true;
        ...
        foreach ($this->fields_form as $k => &$r) {
            ...
            if (isset($_FILTR[$k])) {
                if ($r['type'] == 'date') {
                    ...
                    if (isset($_FILTR[$k . '_2'])) $fields_form['f_' . $k]['value_2'] = $_FILTR[$k . '_2'];
                    $fields_form['f_' . $k]['value'] = $_FILTR[$k];
                }
            }
            if ($r['type'] == 'ajaxlist') {
                if (!isset($fields_form['f_' . $k]['label'])) $fields_form['f_' . $k]['label'] = 'Введите текст';
                ...
                if (isset($_REQUEST['f_' . $k]) && $_REQUEST['f_' . $k] != '') { // && isset($this->fields_form[$k]['mask']['filter'])
                    ...
                    if (!is_array($_REQUEST['f_' . $k])) {
                        if ($row['type'] == 'date') {
                            ...
                            if (isset($_REQUEST['f_' . $k . '_2'])) $_REQUEST['f_' . $k . '_2'] = static_form::_get_fdate($_REQUEST['f_' . $k . '_2'], $row['mask']['format'], $this->fields[$k]['type']);
                        }

                        $_SESSION['filter'][$this->_cl][$k] = $this->SqlEsc($_REQUEST['f_' . $k]);
                        if (isset($_REQUEST['f_' . $k . '_2'])) $_SESSION['filter'][$this->_cl][$k . '_2'] = $this->SqlEsc($_REQUEST['f_' . $k . '_2']);
                    } else {
                    ...
                    else unset($_SESSION['filter'][$this->_cl]['exc_' . $k]);
                } else if (!$flag) unset($_SESSION['filter'][$this->_cl][$k]);
            }
        }
    }
    ...
    function _filter_clause()
    {
        ...
        if (isset($_REQUEST['filter_' . $this->_cl])) {
            if (!is_array($_REQUEST['filter_' . $this->_cl])) unset($_SESSION['filter'][$this->_cl]);
            else $_FILTR = $_REQUEST['filter_' . $this->_cl];
        } elseif (isset($_SESSION['filter'][$this->_cl])) $_FILTR = $_SESSION['filter'][$this->_cl];
                ...
                if (isset($_FILTR[$k]) or isset($_FILTR[$k . '_2'])) {
                    ...
                    if (isset($_FILTR[$k]) and is_array($_FILTR[$k])) {
                        ...
                        if ($r['type'] == 'checkbox') {
                            ...
                            $tmp = array();
                            if (isset($_FILTR[$k]) and $_FILTR[$k] != '') $tmp[] = 't1.' . $k . ($tempex ? '<' : '>') . $_FILTR[$k];
                            if (isset($_FILTR[$k . '_2']) and $_FILTR[$k . '_2'] != '') $tmp[] = 't1.' . $k . ($tempex ? '>' : '<') . $_FILTR[$k . '_2'];
                            if (count($tmp)) $cl[$k] = '(' . implode(($tempex ? ' or ' : ' and '), $tmp) . ')';
                        } elseif ($r['type'] == 'list') {
                            if ($_FILTR[$k] != '') {
                                $cl[$k] = 't1.' . $k . '="' . $this->SqlEsc($_FILTR[$k]) . '"';
                            }
                        } elseif ($_FILTR[$k] == '!0') $cl[$k] = 't1.' . $k . '!=""';
                        elseif ($_FILTR[$k] == '!1') $cl[$k] = 't1.' . $k . '=""';
    ...
    public function _Act($act, &$param)
    {
        $flag = -1;
        $DATA = array();
        if (isset($param['mess'])) $DATA = $param['mess'];
        $param['act'] = $act;
        $this->data = $this->_select();
        if ($this->_prmModulAct($this->data, $param)) {
            $data = array();
            $act = (int)$act;
            if ($this->fields[$this->mf_actctrl]['type'] == 'bool') $data[$this->mf_actctrl] = $act;
            else {
                if (static_main::_prmModul($this->_cl, array(7))) {
                    if ($act == 0) $data[$this->mf_actctrl] = 6;
                    else $data[$this->mf_actctrl] = 1;
                } elseif ($act == 1) $data[$this->mf_actctrl] = 5;
                else $data[$this->mf_actctrl] = 2;
            }

            if ($this->_update($data)) {
                if ($data[$this->mf_actctrl] == 5) $DATA[] = static_main::am('ok', 'act5', $this);
                if ($data[$this->mf_actctrl] == 6) $DATA[] = static_main::am('ok', 'act6', $this);
                elseif ($act) $DATA[] = static_main::am('ok', 'act1', $this);
                else $DATA[] = static_main::am('ok', 'act0', $this);
                $flag = 1;
            } else $DATA[] = static_main::am('error', 'update_err', $this);
        } else $DATA[] = static_main::am('error', 'denied', $this);
        ...
        if (count($this->data) and $this->_prmModulDel($this->data, $param)) {
            if (isset($this->fields[$this->mf_actctrl]) and $this->fields[$this->mf_actctrl]['type'] != 'bool') {
                ...
                if ($this->_update($data)) {
                    ...
                    $flag = 1;
                } else $DATA[] = static_main::am('error', 'del_err', $this);
            } else {
                if ($this->_delete()) {
                    $DATA[] = static_main::am('ok', 'deleted', $this);
                    $flag = 1;
    ...
    public function _ORD($ord, &$param)
    {
        ...
        if ($this->_prmModulEdit($this->data, $param)) {
            ...
            if ($this->_update($data)) {
                if ($ord < 0) $DATA[] = static_main::am('UP', 'ok');
                else $DATA[] = static_main::am('DOWN', 'ok');
                $flag = 1;
            } else $DATA[] = static_main::am('error', 'update_err', $this);
        } else $DATA[] = static_main::am('error', 'denied', $this);
        return array($DATA, $flag);
    ...
    public function _get_file($id, $key, $extValue = '', $modkey = -1)
    {
        ...
        $pref = '';
        if (isset($this->attaches[$key]['thumb'][$modkey]['pref']) && $this->attaches[$key]['thumb'][$modkey]['pref']) $pref = $this->attaches[$key]['thumb'][$modkey]['pref'];

        if (isset($this->attaches[$key]['thumb'][$modkey]['path']) && $this->attaches[$key]['thumb'][$modkey]['path']) $pathimg = $this->attaches[$key]['thumb'][$modkey]['path'] . '/' . $pref . $id . '.' . $extValue;
        elseif (isset($this->attaches[$key]['path']) and $this->attaches[$key]['path']) $pathimg = $this->attaches[$key]['path'] . '/' . $pref . $id . '.' . $extValue;
        else $pathimg = $this->getPathForAtt($key) . $pref . $id . '.' . $extValue;
    ...
    public function _prefixImage($path, $pref)
    {
        if (trim($path) != '') {
            ...
            if (file_exists(SITE . _substr($img, 0, strrpos($img, '?')))) return $img;
        }
        return $path;
    }
    ...
    public function getPathForThumb($imod, $key)
    {
        ...
        if (isset($imod['path']) and $imod['path']) $path = $imod['path'] . '/';
        else $path = $this->getPathForAtt($key) . $imod['pref'];
        return $path;
    }
    ...
    function sendNotif($email = '')
    {
        ...
        if (isset($DATA['data']['item']) and $cnt = count($DATA['data']['item'])) {
            ...
            $MAIL->Send($datamail);
            $result = $this->exec(
                'UPDATE `' . $this->tablename . '` SET ' . $this->mf_notif . '=1 WHERE id IN (' . implode(',', array_keys($DATA['data']['item'])) . ')'
            );
            return '   -sendNotif-  ';
        }
        return '';
