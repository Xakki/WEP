...
class sqlmyi
{
    ...
    function __construct(&$SQL_CFG)
    {
        ...
        if (function_exists('mysqli_connect')) {
            ...
            if (canShowAllInfo() > 1) {
                $_CFG['logs']['sql'][] = array(
                    'query' => '<b>CONNECT - ' . $this->SQL_CFG['host'] . ':' . $this->SQL_CFG['port'] . '</b>', 'time' => $ttt, 'hasError' => false
                );
    ...
    private function sql_connect()
    {
        ...
        if ($this->hlink) mysqli_query($this->hlink, "SET time_zone = '" . date_default_timezone_get() . "'");
    ...
    {
        global $_CFG;
        if (isset($this->SQL_CFG['setnames']) and $this->SQL_CFG['setnames']) mysqli_query($this->hlink, 'SET NAMES ' . $this->SQL_CFG['setnames']);
    ...
    {
        if ($this->hlink) mysqli_close($this->hlink);
    }
...
class myiquery
{
    ...
    { // Выдает асоциативный и нумеровнаый масив
        if ($type == 0) return $this->fetch_assoc();
        elseif ($type == 1) return $this->fetch_row();
        elseif ($type == 2) return $this->fetch_array();
        else return $this->fetch_object();
    }
