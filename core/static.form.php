...
class static_form
{
    static function isSubmited($param)
    {
        ...
        elseif (isset($param['setAutoSubmit'])) $submitFlag = $param['setAutoSubmit'];
        return $submitFlag;
    }
    ...
    static function _add_fields(&$_this, $flag_update = false)
    {
        ...
        foreach ($_this->fld_data as $key => &$value) {
            if (!isset($_this->fields[$key]['noquote'])) {
                ...
                elseif (isset($_this->fields[$key]['secure'])) $value = '\'' . $_this->SqlEsc(static_main::EnDecryptString($value)) . '\'';
                else $value = '\'' . $_this->SqlEsc($value) . '\'';
            }
            if ($flag_update) {
                if (!isset($_this->fields[$key]['noquote'])) $keyData[$key] = '`' . $key . '` = VALUES(`' . $key . '`)';
                else $keyData[$key] = '`' . $key . '` = ' . $value;
            }
        }
        if ($_this->mf_timecr) $_this->fld_data['mf_timecr'] = $_this->_CFG['time'];
    ...
    {
        if (isset($value['ext']) and $value['ext']) return $value['ext'];
        else return $value['ext'] = strtolower(array_pop(explode('.', $value['name'])));
    }
    ...
    static function _add_attaches(&$_this)
    {
        ...
        $result = $_this->SQL->execSQL(
            'SELECT id, ' . implode(',', array_keys($_this->attaches)) . ' FROM `' . $_this->tablename . '` WHERE id IN (' . $_this->_id_as_string() . ')'
        );
        if ($result->err) return false;
    ...
    {
        $res = true;
        if ($imod['type'] == 'crop') $res = static_image::_cropImage($source, $thumb, $imod['w'], $imod['h']);
        elseif ($imod['type'] == 'resize') $res = static_image::_resizeImage($source, $thumb, $imod['w'], $imod['h']); // TODO - IMAGE OPTION normalize
        elseif ($imod['type'] == 'resizecrop' or $imod['type'] == 'thumb' or $imod['type'] == 'resize') $res = static_image::_thumbnailImage($source, $thumb, $imod['w'], $imod['h']);
        elseif ($imod['type'] == 'watermark') $res = static_image::_waterMark($source, $thumb, $imod['logo'], $imod['x'], $imod['y']);
        elseif ($source != $thumb) $res = copy($source, $thumb);

        if ($res) _chmod($thumb);
        else trigger_error('Error chmod: for file ' . $thumb, E_USER_WARNING);
    ...
    static function _add_memos(&$_this)
    {
        ...
        foreach ($_this->mmo_data as $key => $value) {
            ...
            if (!isset($_this->memos[$key])) return static_main::log('error', static_main::m('Error add memo', array($key, $_this->caption)));
            $name = $pathimg . '/' . $_this->id . $_this->text_ext;
            $f = fopen($name, 'w');
            if (!$f) return static_main::log('error', 'Can`t create file ' . $name);
            if (fwrite($f, $value) == -1) return static_main::log('error', 'Can`t write data into file ' . $name);
            if (!fclose($f)) return static_main::log('error', 'Can`t close file ' . $name);
            global $_CFG;
            _chmod($name);
    ...
    static function _update(&$_this, $flag_select = true)
    {
        ...
        if ($_this->mf_timeup) $_this->fld_data['mf_timeup'] = $_this->_CFG['time'];
        if ($_this->mf_timeoff and !isset($_this->fld_data['mf_timeoff']) and isset($_this->fld_data[$_this->mf_actctrl]) and !$_this->fld_data[$_this->mf_actctrl] and $_this->data[$_this->id][$_this->mf_actctrl]) $_this->fld_data['mf_timeoff'] = $_this->_CFG['time'];
        if ($_this->mf_ipcreate) {
    ...
    static function _update_fields(&$_this, $where)
    {
        ...
        foreach ($_this->fld_data as $key => $value) {
            if (!isset($_this->fields[$key]['noquote'])) {
                ...
                } elseif (strpos($_this->fields[$key]['type'], 'bool')) $value = (int)(bool)$value;
                elseif (strpos($_this->fields[$key]['type'], 'int') !== false) $value = str2int($value);
                elseif (strpos($_this->fields[$key]['type'], 'float') !== false) $value = floatval($value);
                elseif (isset($_this->fields[$key]['secure'])) $value = '\'' . $_this->SqlEsc(static_main::EnDecryptString($value)) . '\'';
                else $value = '\'' . $_this->SqlEsc($value) . '\'';
            }
            if ($_this->sql_quote) {
        ...
        if (isset($_this->fld_data[$_this->mf_istree]) and !is_array($_this->id)) $_this->parent_id = $_this->fld_data[$_this->mf_istree];
    ...
    static function _rename_childs(&$_this)
    {
        ...
        foreach ($_this->childs as $ch => $child) {
            $result = $_this->SQL->execSQL(
                'UPDATE `' . $_this->childs[$ch]->tablename . '` SET ' . $_this->childs[$ch]->owner_name . ' = \'' . $_this->fld_data['id'] . '\' WHERE ' . $_this->childs[$ch]->owner_name . ' =\'' . $_this->id . '\''
            );
            if ($result->err) return false;
        }
        return true;
    ...
    static function _rename_parent_childs(&$_this)
    {
        ...
        $result = $_this->SQL->execSQL(
            'UPDATE `' . $_this->tablename . '` SET `parent_id` = \'' . $_this->fld_data['id'] . '\' WHERE parent_id =\'' . $_this->id . '\''
        );
        if ($result->err) return false;
        return true;
    }
    ...
    static function _rename_attaches(&$_this)
    {
        ...
        if ($row) {
            foreach ($_this->attaches as $key => $value) {
                ...
                if (file_exists($f)) rename($f, $pathimg . $_this->fld_data['id'] . '.' . $value['exts'][$row[$key]]);
            }
        }
        return true;
    }
    ...
    private static function _delete_attaches(&$_this, array $id)
    {
        if (!count($_this->attaches)) return true;
        $result = $_this->SQL->execSQL(
            'SELECT `id`, `' . implode('`,`', array_keys($_this->attaches)) . '` FROM `' . $_this->tablename . '` WHERE `id` IN (' . $_this->_as_string($id) . ')'
        );
        if ($result->err) return false;
    ...
    {
        $eval = '';
        if (isset($ff['mask']['eval'])) $eval = $ff['mask']['eval'];
        elseif (isset($ff['mask']['evala']) and !$_this->id) $eval = $ff['mask']['evala'];
        elseif (isset($ff['mask']['evalu']) and $_this->id) $eval = $ff['mask']['evalu'];
        return $eval;
    }
        ...
        foreach ($fields as $k => &$r) {
            ...
            if (isset($r['type']) and $r['type'] != 'info') {
                if (!isset($r['value']) and isset($r['default']) and !isset($_POST[$k])) { // and !$_this->id
                    $r['value'] = $r['default'];
                    if (isset($r['default_2'])) $r['value_2'] = $r['default_2'];
                }
...
//									}
                    ...
                    if (!isset($r['value']) or !is_string($r['value']) or !$r['value']) $r['value'] = '';

                    if (!isset($r['comment'])) $r['comment'] = static_main::m('_file_size') . $_this->attaches[$k]['maxsize'] . 'Kb';
                } elseif ($r['type'] == 'ajaxlist') {
                    if (!isset($r['placeholder']) or !$r['placeholder']) $r['placeholder'] = 'Введите текст';
                    if (isset($r['mask']['min']) and $r['mask']['min'] and (!isset($r['value']) or $r['value'] < $r['mask']['min'])) $r['value_2'] = '';
                    ...
                    if ((!isset($r['value_2']) or !$r['value_2']) and isset($r['value']) and $r['value']) {
                        ...
                        if (isset($r['multiple'])) {
                            foreach ($r['value'] as $kv => $rv) $r['value_2'][$kv] = (isset($md[$rv]) ? $md[$rv] : '');
    ...
    if(is_array($md) and count($md)) {
        ...
        $r['valuelist'] = $_this->_forlist($md, $key, $r['value'], $r['multiple']);
                    ...
                    if (!$r['readonly']) {
                        ...
                        if (!isset($r['multiple'])) $r['multiple'] = 0;

                        $md = $_this->_getCashedList($r['listname']);

                        if (!isset($r['value'])) $r['value'] = array();
                        elseif (!is_array($r['value'])) $r['value'] = array($r['value'] => $r['value']);
                        elseif (is_array($r['value']) and count($r['value']) and $r['multiple'] != FORM_MULTIPLE_KEY) $r['value'] = array_combine($r['value'], $r['value']);

                        if (is_array($md) and count($md)) {
                            $temp = current($md);
                            if (is_array($temp) and !isset($temp['#name#'])) {
                                if (isset($r['mask']['begin'])) $key = $r['mask']['begin']; //стартовый ID массива
                                else $key = key($md);
                            } else {
                                $md = array($md);
                                $key = 0;
                    ...
                    } else // Форма списка  только для чтения, выводится в виде текста
                    {
                        ...
                        if (isset($r['value']) and $r['value'] != '') {
                            ...
                            if (is_array($selectedData)) {
                                ...
                                if (isset($temp['#name#'])) {
                                    $r['value'] = array();
                                    foreach ($selectedData as $listname) $r['value'][] = $listname['#name#'];
                                    $r['value'] = implode(',', $r['value']);
                                } else $r['value'] = implode(',', $selectedData);
                            } else $r['value'] = $selectedData;
                        }
                    }
                } elseif ($r['type'] == 'ckedit') {
                    if (!isset($r['paramedit'])) $r['paramedit'] = array();
    ...
    { //$_this->fields_form
        ...
        foreach ($FORMS_FIELDS as $key => &$form) {
            ...
            if ($key == '_*features*_') {
                trigger_error(
                    'Ошибка. В форме модуля ' . $_this->_cl . ', применяются устаревший _*features*_',
                    E_USER_WARNING
                );
                continue;
            }
            if (!isset($form['type'])) return array(
                'mess' => array(
                ...
                elseif (isset($data[$key])) {
                    if (is_array($data[$key])) {
                        ...
                        if (isset($form['mask']['minarr'])) {
                            if (count($data[$key]) < $form['mask']['minarr']) $error[] = 27;
                        }
            ...
            foreach ($error as $row) {
                ...
                elseif ($row == 3) { //wrong data
                    if (isset($form['matches_err']) and count($form['matches_err'][0])) {
                        ...
                        foreach ($form['matches_err'][0] as $mk => $mr) {
                            if (isset($mr[1])) $textm .= $mr[0] . '(поз. ' . $mr[1] . '), ';
                            if ($mk > 10) {
                                $textm .= 'и т.д., ';
                                break;
                            }
                ...
                if (static_form::isSubmited($param) == 2) { //  если не автосабмит
                    ...
                    if ($form['type'] == 'hidden') {
                        ...
                        trigger_error(
                            'Ошибка в элементе hidden, формы модуля ' . $_this->_cl . ' : `' . $form['caption'] . '` ' . $messages,
                            E_USER_WARNING
                        );
                    } else {
        ...
        if ($form['type'] == 'file') {
            ...
            if (isset($data[$key . '_del']) and (int)$data[$key . '_del'] == 1) {
                ...
                if ($value['error'] != 0) {
                    ...
                    if ($is_image) {
                        $form['att_type'] = 'img';
                        if ((!isset($form['toWebImg']) or $form['toWebImg']) and $is_image > 3) $value['ext'] = (is_string($form['toWebImg']) ? $form['toWebImg'] : 'jpg');
                        else $value['ext'] = static_image::_get_ext($is_image, false);
                    } else {
                        if ($value['name']) $value['ext'] = strtolower(array_pop(explode('.', (string)$value['name'])));
    ...
    static function passwordHash($val, $form)
    {
        ...
        if (isset($form['md5']) and !$form['md5']) return $val;
    ...
    static function check_formfield(&$_this, &$form, &$error, &$data, $key)
    {
        ...
        if ($form['type'] == 'password') {
            if (isset($form['mask']['password']) and $form['mask']['password'] == 're') {
                if ($data[$key] or $data['re_' . $key]) {
                    if ($data[$key] != $data['re_' . $key]) $error[] = 32;
                    else $data[$key] = self::passwordHash($data[$key], $form);
                } else unset($data[$key]);
            ...
            elseif (isset($form['mask']['password']) and $form['mask']['password'] == 'confirm') {
                if (isset($_this->data[$_this->id][$key]) and $data[$key]) {
                    if ($_this->data[$_this->id][$key] != self::passwordHash($data[$key], $form)) $error[] = 322;
                    unset($data[$key]);
                }
            } // форма смены пароля
            elseif (isset($form['mask']['password']) and $form['mask']['password'] == 'change') {
                if (isset($_this->data[$_this->id][$key]) and $data[$key] or $data[$key . '_old']) {
                    if ($_this->data[$_this->id][$key] != self::passwordHash($data[$key . '_old'], $form)) $error[] = 321;
                    else $data[$key] = self::passwordHash($data[$key], $form);
                }
                unset($data[$key]);
            } else {
                if (isset($form['mask']['max']) && $form['mask']['max'] > 0 && _strlen($data[$key]) > $form['mask']['max']) $error[] = 2;
                if (isset($form['mask']['min']) and $form['mask']['min'] > 0) {
                    if (!$data[$key] or $data[$key] == '0') $error[] = 1;
                    elseif (_strlen($data[$key]) < $form['mask']['min']) $error[] = 21;
                }
        ...
        if (!isset($data[$key])) {
            if (isset($form['mask']['min']) and $form['mask']['min']) $error[] = 1;
            return true;
        }
        ...
        if (isset($form['mask']['replace'])) {
            if (!isset($form['mask']['replaceto'])) $form['mask']['replaceto'] = '';
            $data[$key] = preg_replace($form['mask']['replace'], $form['mask']['replaceto'], $data[$key]);
        }

        /*Убираем теги*/
        if (isset($form['mask']['striptags'])) {
            if ($form['mask']['striptags'] == 'all') $data[$key] = strip_tags($data[$key]);
            elseif ($form['mask']['striptags'] == '') $data[$key] = strip_tags($data[$key], $_this->_CFG['_striptag']);
            else $data[$key] = strip_tags($data[$key], $form['mask']['striptags']);
        }
        ...
        if ($preg_mask AND $data[$key]) {
            ...
            if (is_array($preg_mask)) {
                ...
                if (isset($preg_mask['nomatch'])) $nomatch = $preg_mask['nomatch'];
                if (isset($preg_mask['comment'])) $form['comment'] .= $preg_mask['comment'];
            } else $nomatch = $preg_mask;
            ...
            if (isset($form['mask']['name']) and ($form['mask']['name'] == 'phone' or $form['mask']['name'] == 'phone2')) {
                ...
                if ($matches) {
                    $error[$key . 'mask'] = 3;
                } elseif (isset($form['mask']['checkwww']) and $form['mask']['checkwww'] and !fopen('http://' . str_replace('http://', '', $data[$key]), 'r')) $error[] = 4;
            }
        }
        ...
        if (isset($form['mask']['max']) && $form['mask']['max'] > 0) {
            if (self::isTypeNumber($form['fields_type'])) {
                ...
                if (str2int($data[$key]) > $form['mask']['max']) $error[] = 22;
            } else {
                if (_strlen($data[$key]) > $form['mask']['max']) $error[] = 2;
            }
        }
        /*
		...
		*/
        if (isset($form['mask']['min']) and $form['mask']['min'] > 0) {
            ...
            elseif (self::isTypeNumber($form['fields_type'])) {
                ...
                if (str2int($data[$key]) < $form['mask']['min']) $error[] = 23;
            } else {
                if (_strlen($data[$key]) < $form['mask']['min']) $error[] = 21;
            }
        }
    ...
    static function _get_fdate($inp_date, $format, $field_type)
    {
        ...
        $ci = count($inp_date);
        if ($ci == 1) {
            if ($field_type == 'int') return $inp_date[0];
    ...
    static function setCaptcha($mask)
    {
        ...
        $A = array(
            ...
            1 => array(
                1, 2, 4, 5, 6, 7, 8, 9, 'А', 'Б', 'В', 'Г', 'Д', 'Е', 'Ж', 'И', 'К', 'Л', 'М', 'Н', 'П', 'Р', 'С', 'Т',
                'У', 'Ф', 'Х', 'Ц', 'Ч', 'Э', 'Ю', 'Я'
            ),
            2 => array(
                1, 2, 4, 5, 6, 7, 8, 9, 'а', 'б', 'в', 'г', 'д', 'е', 'ж', 'и', 'к', 'л', 'м', 'н', 'п', 'р', 'с', 'т',
            ...
            3 => array(
                1, 2, 4, 5, 6, 7, 8, 9, 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T',
                'U', 'V', 'W', 'X', 'Y', 'Z'
            ),
            4 => array(
                1, 2, 4, 5, 6, 7, 8, 9, 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'm', 'n', 'p', 'q', 'r', 's', 't', 'u',
                'v', 'w', 'x', 'y', 'z'
            ),
            5 => array(
                1, 2, 4, 5, 6, 7, 8, 9, 'А', 'Б', 'В', 'Г', 'Д', 'Е', 'Ж', 'И', 'К', 'Л', 'М', 'Н', 'П', 'Р', 'С', 'Т',
                'У', 'Ф', 'Х', 'Ц', 'Ч', 'Э', 'Ю', 'Я', 'а', 'б', 'в', 'г', 'д', 'е', 'ж', 'и', 'к', 'л', 'м', 'н',
                'п', 'р', 'с', 'т', 'у', 'ф', 'х', 'ц', 'ч', 'э', 'ю', 'я'
            ),
            6 => array(
                1, 2, 4, 5, 6, 7, 8, 9, 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T',
                'U', 'V', 'W', 'X', 'Y', 'Z', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'm', 'n', 'p', 'q', 'r', 's', 't',
                'u', 'v', 'w', 'x', 'y', 'z'
            ),
        );
        $word = '';
        ...
        if (isset($_COOKIE['chash']) and $_COOKIE['chash']) {
            ...
            if (function_exists('openssl_encrypt')) {
                ...
                $word = mcrypt_decrypt(
                    MCRYPT_RIJNDAEL_256,
                    $hash_key,
                    base64decode($_COOKIE['chash']),
                    MCRYPT_MODE_ECB,
                    mcrypt_create_iv(mcrypt_get_iv_size(MCRYPT_RIJNDAEL_256, MCRYPT_MODE_ECB), MCRYPT_RAND)
                );
                $word = trim($word);
            } else $word = $_COOKIE['chash'];
            return $word;
        }
    ...
    static function SpellCheck($txt)
    {
        ...
        $txt2 = preg_replace(
            array(
                ...
                '#(.)(\\1){3,}#iu', // прочие повторяющиеся не символы
    ...
    static function isTypeBool($type)
    {
        ...
        if (isset($list[$type])) return true;
        return false;
    }
