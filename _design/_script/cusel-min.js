...
{classDisCuselText="classDisCuselText";classDisCusel="classDisCusel";}
if(scrollArrows)
...
var arrAddTags=jQuery("#cusel-scroll-"+chElId).find("span[addTags]"),lenAddTags=arrAddTags.length;for(i=0;i<lenAddTags;i++)arrAddTags.eq(i).append(arrAddTags.eq(i).attr("addTags")).removeAttr("addTags");});jQuery("html").unbind("mousedown");jQuery("html").mousedown(function(e)
{var clicked=jQuery(e.target),clickedId=clicked.attr("id"),clickedClass=clicked.attr("class");if(clickedClass.indexOf("cuselText")!=-1&&clickedClass.indexOf("classDisCuselText")==-1)
{var cuselWrap=clicked.next();if(cuselWrap.css("display")=="none")
{jQuery(".cusel-scroll-wrap").css("display","none");cuselWrap.css("display","block");var cuselArrows=false;if(clicked.parents(".cusel").attr("class").indexOf("cuselScrollArrows")!=-1)cuselArrows=true;if(!cuselWrap.find(".jScrollPaneContainer").eq(0).is("div"))cuselWrap.find("div").eq(0).jScrollPaneCusel({showArrows:cuselArrows});}
else
...
{jQuery(".cusel-scroll-wrap").css("display","none");cuselWrap.css("display","block");var cuselArrows=false;if(clicked.attr("class").indexOf("cuselScrollArrows")!=-1)cuselArrows=true;if(!cuselWrap.find(".jScrollPaneContainer").eq(0).is("div"))cuselWrap.find("div").eq(0).jScrollPaneCusel({showArrows:cuselArrows});}
else
{cuselWrap.css("display","none");}}
else if(clicked.is(".cusel-scroll-pane span"))
{clicked.parents(".cusel-scroll-wrap").find(".cuselActive").eq(0).removeClass("cuselActive").end().parents(".cusel-scroll-wrap").next().val(clicked.attr("value")).end().prev().text(clicked.text()).end().css("display","none");clicked.addClass("cuselActive");clicked.parents(".cusel").find("input").eq(0).change();}
