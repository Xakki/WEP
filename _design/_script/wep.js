...
if (isProduction) {
    ...
    };//opera.postError(comm);
    console.error = console.error || function () {
        opera.postError(arguments);
    };
    console.assert = console.assert || function () {
        if (!arguments[0]) opera.postError(arguments);
    };
...
window.wep = {
    ...
    ajaxLoadContent: function (domObject, paramCustom) {
        ...
        if (paramCustom && paramCustom['data'])
    ...
    showHelp: function (obj, mess, time, nomiga) {
        ...
        setTimeout(function () {
            jQuery(slct).stop().fadeOut(300, function () {
                jQuery(slct).remove()
            });
        }, time);
        //if(time>5000)
    ...
    miga: function (obj, opc1, opc2) {
        ...
        if (opc == opc2)
            ...
            opc = opc2;
        jQuery(obj).animate({'opacity': opc}, 1000, function () {
            miga(obj, opc1, opc2);
        });
        return false;
    },
    ...
    checkCSSInclude: function (url) {
        url = wep.absPath(url);
        if (jQuery.isEmptyObject(wep.includeCSSlist)) {// проверка на уникальность подключаемого стиля
    ...
    checkJsInclude: function (url) {
        url = wep.absPath(url);
        if (jQuery.isEmptyObject(wep.includejslist)) {// проверка на уникальность подключаемого
